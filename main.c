...
{
    uint32_t radar_hits = 0; /* Avionics theme: track radar pulses */
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
            ...
            radar_hits++;
            ESP_LOGI(TAG, "[UAV_RADAR_SEM] Pulse received! Hit #%lu | latency=%lld us (max=%llu)",
                     (unsigned long)radar_hits,
                     (long long)lat,
...
{
    uint32_t beacon_updates = 0; /* Avionics theme: track beacon status updates */
    for (;;) {
        ...
        ESP_LOGI(TAG, "[UAV_BEACON_NOTIF] Status updated! Total updates=%lu | latency=%lld us (max=%llu) (notif count=%lu)",
                 (unsigned long)beacon_updates,
                 (long long)lat,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [AVIONICS/DEFENSE UAV] starting — ISR + bottom-half ====");
